<template>
	<section>
		...
		<!--列表-->
		<el-table :data="noteList" v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
...
<script>
	...
	import {getNoteList,delNote,getTypeList} from '../../api/api.js'
	export default {
		data() {
			return {
				noteList:[],
		...
		methods: {
			//获取列表
			getList(){
				getNoteList(this.params).then(res => {	
			...
			//编辑
			handleEdit(row){
				this.$router.push(`/noteedit/${row.id}`)
			},
			//删除
